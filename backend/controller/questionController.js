...
async function singleQuestion(req, res) {
  ...
  const [singleQuestion] = await dbConnection.query(
    `SELECT q.id,q.questionid,q.userid ,q.title,q.description,q.tag,u.username, u.firstname, u.lastname, u.profession FROM questions q join users u on q.userid = u.userid WHERE q.questionid = '${req.params.question_id}' order by q.id desc `
  )
...
async function getAllQuestion(req, res) {
  const [allQuestion] = await dbConnection.query(
    "SELECT q.id,q.questionid,q.userid ,q.title,q.description,q.tag,u.username, u.firstname, u.lastname, u.profession FROM questions q join users u on q.userid = u.userid order by q.id desc "
  )
...
async function getSeachedQuestion(req, res) {
  ...
  const [allQuestion] = await dbConnection.query(
    `SELECT q.id,q.questionid,q.userid ,q.title,q.description,q.tag,u.username, u.firstname, u.lastname, u.profession FROM questions q join users u on q.userid = u.userid WHERE title like '%${req.params.search}%' or description like '%${req.params.search}%' order by q.id desc `
  )
  console.log(allQuestion)
  return res.status(StatusCodes.OK).json({
    msg: "Question/s retrieved successfully ",
...
async function updateQuestion(req, res) {
  ...
  if (title === undefined && description === undefined && tag === undefined) {
    return res.status(StatusCodes.BAD_REQUEST).json({
      message:
        "At least one field (title, description, or tag) must be provided to update",
    });
  }
  ...
  try {
    ...
    if (question.length === 0) {
      return res.status(StatusCodes.FORBIDDEN).json({
        message:
          "You are not allowed to edit this question or it doesn't exist",
      });
    }

    // Update only fields that are provided, keep others unchanged
    await dbConnection.query(
