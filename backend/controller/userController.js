...
const register = async (req, res) => {
  ...
  try {
    ...
    // Insert user with role_id and profession and get the inserted ID
    ...
    const token = jwt.sign(
      { userid, username, role_id }, // You can add more user details here if needed
    ...
    return res.status(StatusCodes.CREATED).json({
      ...
      user: { // Include user details in the response for convenience
        userid,
        username,
        firstname,
        lastname,
...
const login = async (req, res) => {
  ...
  try {
    ...
    const [user] = await DBConnection.query(
      `SELECT u.userid, u.username, u.password, r.role_name, r.role_id, u.firstname, u.lastname, u.profession 
       FROM users u
    ...
    const { userid, username, role_id, role_name, firstname, lastname, profession } = user[0];
    const token = jwt.sign(
